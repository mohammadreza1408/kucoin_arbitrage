...
def get_enable_coin(url):
    pass


# Make a get request
def get_coin_tickers(url):
    req = requests.get(url + f'/api/v1/market/allTickers')
    json_resp =  req.json()['data']['ticker']
    return json_resp


# Loop through each objects and find the tradeable pairs    
def collect_tradeable(json_object):

    req = requests.get('https://api.kucoin.com/api/v2/symbols')
    json_resp =  req.json()['data']
    disable_coin_list=[]
    for disable_coin in range(len(json_resp)):
        if json_resp[disable_coin]['enableTrading'] == 'false': ### Specifies inactive coins
            disable_coin_list.append(disable_coin['symbol'])



    coin_list = []
    for i in range(len(json_object)):
        if json_object[i]['symbol'] not in disable_coin_list:
            coin_list.append(json_object[i]['symbol'])
    return coin_list
...
def structure_triangular_pair(coin_list):
    ...
    for pair_a in pairs_list:
        ...
        for pair_b in pairs_list:
            ...
            # Check pair B  
            if pair_b != pair_a: 
                if b_base in a_paire_box or b_quote in a_paire_box: 
                    ...
                    for pair_c in pairs_list:
                        ...
                        # Count the numbers of matching C items  
...
def get_price_for_t_pair(t_pair, prices_json):
    new_prices_json = {item['symbol']: item for item in prices_json}  
    ...
    pair_a_ask = float(new_prices_json[pair_a]["buy"])
    pair_a_bid = float(new_prices_json[pair_a]["sell"])

    pair_b_ask = float(new_prices_json[pair_b]["buy"])
    pair_b_bid = float(new_prices_json[pair_b]["sell"])

    pair_c_ask = float(new_prices_json[pair_c]["buy"])
    pair_c_bid = float(new_prices_json[pair_c]["sell"])
...
def calc_triangular_arb_surface_rate(t_pair, prices_dict):
    ...
    for direction in direction_list:
        ...
        """FORWARD"""
        # SCENARIO 1 Cheack if a_quote (acquired_coin) matches b_quote
        if direction == "forward":
        ...
        # SCENARIO 2 Cheack if a_quote (acquired_coin) matches b_base
        if direction == "forward":
        ...
        # SCENARIO 3 Cheack if a_quote (acquired_coin) matches c_quote
        if direction == "forward":
        ...
        # SCENARIO 4 Cheack if a_quote (acquired_coin) matches c_base
        if direction == "forward":
        ...
        """REVERSE"""
        # SCENARIO 1 Cheack if a_base (acquired_coin) matches b_quote
        if direction == "reverse":
        ...
        # SCENARIO 2 Cheack if a_base (acquired_coin) matches b_base
        if direction == "reverse":
        ...
        # SCENARIO 3 Cheack if a_base (acquired_coin) matches c_quote
        if direction == "reverse":
        ...
        # SCENARIO 4 Cheack if a_base (acquired_coin) matches c_base
        if direction == "reverse":
...
def get_depth_from_orderbook(surface_arb):
    ...
    # Get Order Book for First Trade Assessments
    url1 = f"https://api.kucoin.com/api/v1/market/orderbook/level2_20?symbol={contract_1}"
    req1 = requests.get(url1)
    responce1 = req1.json()
    depth_1_price = responce1['data']
    depth_1_reformatted_prices = reformated_orderbook(depth_1_price,contract_1_direction)
    time.sleep(0.3)
    url2 = f"https://api.kucoin.com/api/v1/market/orderbook/level2_20?symbol={contract_2}"
    req2 = requests.get(url2)
    responce2 = req2.json()
    depth_2_price = responce2['data']
    depth_2_reformatted_prices = reformated_orderbook(depth_2_price,contract_2_direction)
    time.sleep(0.3)
    url3 = f"https://api.kucoin.com/api/v1/market/orderbook/level2_20?symbol={contract_3}"
    req3 = requests.get(url3)
    responce3 = req3.json()
    depth_3_price = responce3['data']
    depth_3_reformatted_prices = reformated_orderbook(depth_3_price,contract_3_direction)
